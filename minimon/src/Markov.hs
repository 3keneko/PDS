...
import Numeric.LinearAlgebra (toList, Matrix, (><), diag, eig, inv, cmap, disp, vector, (#>))
...
multALot n = cmap realPart res
  where (l, v) = eig ((18 >< 18) (map snd $ sortBy (compare `on` fst) $ map (\(x, y, z) -> (x + 18 * y, z)) graphAsTriple))
...
markovPrediction n =
  let res = multALot n  #> vector (replicate 18 1)
      ...
      v = sortBy (flip compare `on` snd) o
  in do
